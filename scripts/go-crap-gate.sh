...
#
# Run from within the Go module directory being gated (each eos-plugins sink
# is its own module) so `go-crap scan .` and the git diff pathspecs stay
...
CHANGED_GO="$(git diff --relative --name-only "$DIFF_BASE" HEAD -- '*.go' | grep -v '_test\.go$' || true)"
...
changed_files = [
    f for f in subprocess.run(
        ["git", "diff", "--relative", "--name-only", diff_base, "HEAD", "--", "*.go"],
